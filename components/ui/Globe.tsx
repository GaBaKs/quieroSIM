...
export default function Globe({
  className,
  size = 600,
  dotColor = "rgba(153, 51, 193, ALPHA)", // #9933c1
  arcColor = "rgba(153, 51, 193, 0.5)", // #9933c1
  markerColor = "rgba(153, 51, 193, 1)", // #9933c1
  ...
  useEffect(() => {
    if (focusCoordinates) {
      ...
      const targetY = (90 - lng) * (Math.PI / 180);
  ...
  const draw = useCallback(() => {
    ...
    // Auto rotate or seek target
    if (!dragRef.current.active) {
      if (targetRotRef.current) {
        const tx = targetRotRef.current.x;
        const ty = targetRotRef.current.y;
        
        const diffY = ty - rotYRef.current;
        const shortestDiffY = Math.atan2(Math.sin(diffY), Math.cos(diffY));
        const diffX = tx - rotXRef.current;

        rotYRef.current += shortestDiffY * 0.05;
        rotXRef.current += diffX * 0.05;

        // slowly drift the target so the globe stays alive
        targetRotRef.current.y += autoRotateSpeed * 0.2;
      } else {
    ...
    glowGrad.addColorStop(0, "rgba(153, 51, 193, 0.03)"); // #9933c1
    glowGrad.addColorStop(1, "rgba(153, 51, 193, 0)");
    ...
    ctx.strokeStyle = "rgba(153, 51, 193, 0.06)";
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    for (const marker of markers) {
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = "rgba(24, 24, 27, 0.8)"; // text-zinc-900 with some opacity
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn("w-full h-full pointer-events-none md:pointer-events-auto md:cursor-grab md:active:cursor-grabbing", className)}
