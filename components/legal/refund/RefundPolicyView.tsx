...
import RefundPolicyES from './RefundPolicyES';
import RefundPolicyEN from './RefundPolicyEN';
import RefundPolicyPT from './RefundPolicyPT';

export default function RefundPolicyView() {
  const { lang, t } = useLanguage();

  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 antialiased" id="refund-policy-view">
      {/* Mini header/Navbar */}
      ...
      {lang === 'ES' && <RefundPolicyES />}
      {lang === 'EN' && <RefundPolicyEN />}
      {lang === 'PT' && <RefundPolicyPT />}
    </div>
  );
}
